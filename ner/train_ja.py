...
# 전처리 객체 생성
p = Preprocess_ja(word2index_dic='./train_tools/dict/chatbot_dict_ja.bin',
            userdic=None)

def train_ja():
    # 학습용 말뭉치 데이터를 불러옴
    corpus = read_file('./ner/ner_train_ja.txt')
    ...
    model.summary()
    model.fit(x_train, y_train, batch_size=128, epochs=5,  callbacks = [callback])

    print("평가 결과 : ", model.evaluate(x_test, y_test)[1])
    model.save('ner/ner_model_ja_ver1.h5')
